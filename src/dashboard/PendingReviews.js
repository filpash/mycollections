...
const styles = {
    ...
    icon: { float: 'right', width: 64, height: 64, padding: 8, color: '#f44336' },
};

const location = { pathname: 'reviews', query: { filter: JSON.stringify({ status: 'accepted' }) } };

export default translate(({ reviews = [], customers = {}, nb, translate }) => (
    <Card style={styles.card}>
        <CommentIcon style={styles.icon} />
        <CardTitle title={<Link to={location} style={styles.titleLink}>{nb}</Link>}
